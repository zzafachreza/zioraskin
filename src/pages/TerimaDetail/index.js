...
export default function TerimaDetail({ navigation, route }) {

    const [data, setData] = useState({});
    const isFocused = useIsFocused();

    const getDetail = () => {
        axios.post(apiURL + 'terima_detail', {
            id: route.params.id
        }).then(res => {
            setLoading(false);
        ...
        <SafeAreaView style={{
            ...
            {!loading && <View style={{
                ...
                <ScrollView showsVerticalScrollIndicator={false} style={{
                    ...
                    <View style={{
                        ...
                        <MYlistdata label="Stok Datang" value={data.qty_datang} />
            ...
            {!loading && <View style={{
                ...
                <View style={{
                    ...
                    <MyButton onPress={() => navigation.navigate('TerimaEdit', data)} title="Edit" Icons="create-outline" />
                    ...
                    <MyButton onPress={() => {
                        Alert.alert(MYAPP, 'Apakah kamu yakin akan hapus ini ?', [
                            { text: 'TIDAK' },
                            {
                                ...
                                onPress: () => {
                                    setLoading(true);
                                    axios.post(apiURL + 'terima_hapus', {
                                        id: data.id
                                    }).then(res => {
                                        setTimeout(() => {
